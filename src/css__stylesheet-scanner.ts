...
// TO DO: Detect if there are other stylesheets added or removed later.
// Specify list of properties that affect hight contrast mode
{
    ...
    function iterate(root: Element, stylesheets: CSSStyleSheet[]) {
    ...
    const observer = new MutationObserver(() => iterate(root, stylesheets));

    iterate(root, stylesheets);
}